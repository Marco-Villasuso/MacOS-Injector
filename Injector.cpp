...
int main() {
    ...
    printf("~~ Injecting; please wait. ~~\r\n\r\n");
    std::vector<int> list = getPids("TargetProcess");
    if (list.empty()) {
        std::cerr << "No target process found.\n";
        return 1;
    }
    
    int targetProcModule = list[list.size()-1];  // The actual module will always be the last process
    ...
    std::string cmd = spath + "./ShellInject.sh " + std::to_string(targetProcModule);
